...
local function ensure_note_hl()
  if fn.hlexists 'NumHiNoteSign' == 0 then
    api.nvim_set_hl(0, 'NumHiNoteSign', {
      fg = '#ffaa00',
      bg = 'NONE',
    })
  end
  if fn.hlexists 'NumHiNoteVirt' == 0 then
    api.nvim_set_hl(0, 'NumHiNoteVirt', {
      fg = '#ffaa00',
      bg = 'NONE',
    })
  end
...
local function get_note(buf, id)
...
local function load_metadata(buf)
  ...
  for _, m in ipairs(marks) do
    ...
    local id = api.nvim_buf_set_extmark(buf, ns, sr, sc, {
      ...
      sign_text = '✎',
      sign_hl_group = hl,
      virt_text = (m.tags and #m.tags > 0) and {
        {
          '#' .. table.concat(m.tags, ' #'),
          hl,
        },
    ...
    if m.label then
      State.labels[m.pal][m.slot] = m.label
    end
  end
end
  ...
  api.nvim_create_autocmd('BufReadPost', {
    callback = function(ev)
      vim.schedule(function()
        load_metadata(ev.buf)
      end)
    end,
  })
  api.nvim_create_autocmd('BufWritePost', {
    callback = function(ev)
      if api.nvim_buf_get_option(ev.buf, 'buftype') == '' then
        save_metadata(ev.buf)
      end
    end,
  })
end
...
local function word_range()
  ...
  while e < #line - 1 and line:sub(e + 2, e + 2):match '[%w_]' do
    e = e + 1
  end
  return s, e + 1
end
...
local function get_label(pal, slot)
  ...
  if not label then
    vim.ui.input({ prompt = ('NumHi %s-%d label (empty = none): '):format(pal, slot) }, function(input)
      if input and input ~= '' then
        State.labels[pal][slot] = input
      end
    end)
  end
...
function C.highlight(slot)
  slot = tonumber(slot)
  if not slot or slot < 1 or slot > 99 then
    return
  end
  ...
  local id = api.nvim_buf_set_extmark(0, ns, start_row, start_col, {
    end_row = end_row,
    end_col = end_col,
    hl_group = group,
  })
  ...
  if #State.history > State.opts.history_max then
...
function C.erase_under_cursor()
  ...
  local ids = api.nvim_buf_get_extmarks(0, ns, {
    l - 1,
    c,
  }, {
    l - 1,
  ...
  for _, m in ipairs(ids) do
    ...
    table.insert(
      marks,
      store_mark(0, id, tonumber(det.hl_group:match '_(%d+)$'), sr, sc, det.end_row, det.end_col, note_tbl and note_tbl.note, note_tbl and note_tbl.tags)
    )
...
local function recreate_mark(mark, pal)
  ...
  local id = api.nvim_buf_set_extmark(buf, ns, sr, sc, {
    ...
    end_col = ec,
    hl_group = hl,
    sign_text = (note and '✎' or nil),
    sign_hl_group = hl,
    virt_text = (tags and #tags > 0) and {
      {
        tags_as_string(tags),
        hl,
      },
...
function C.cycle_palette(step)
  ...
  local chunks = {
    {
      'NumHi → palette ' .. State.active_palette .. '  ',
      'ModeMsg',
    },
  }
  for n = 1, 10 do
...
function C.show_label_under_cursor()
  local l, c = unpack(api.nvim_win_get_cursor(0))
  for _, pal in ipairs(State.opts.palettes) do
    local marks = api.nvim_buf_get_extmarks(0, ns_ids[pal], {
      l - 1,
      c,
    }, { l - 1, c + 1 }, { details = true, overlap = true })
    if #marks > 0 then
      local id = marks[1][1]
...
function C.edit_note()
  ...
  for _, pal in ipairs(State.opts.palettes) do
    local ns = ns_ids[pal]
    local marks = api.nvim_buf_get_extmarks(0, ns, {
      l - 1,
      c,
    }, {
      l - 1,
    ...
    if #marks > 0 then
      ...
      local note_tbl = get_note(0, id) or {
        note = '',
        tags = {},
      }
      ...
      -- 'q' to write & quit
      api.nvim_buf_set_keymap(buf, 'n', 'q', '<cmd>write | close<CR>', {
        silent = true,
      })
      return
    end
