...
local function ensure_hl(
  ...
  local group = ("NumHi_%s_%d"):format(
    pal,
    slot
  )
  if
...
local function line_len(
  buf,
  l
)
  local txt = api.nvim_buf_get_lines(
    buf,
    l,
    l + 1,
    true
...
local function get_note(
  buf,
  id
)  return note_store(buf)[id]      end
local function set_note(
  buf,
  id,
  note,
  tags
...
local function save_metadata(buf)
  ...
  for pal, ns in pairs(ns_ids) do
    local em = api.nvim_buf_get_extmarks(
      buf,
      ns,
      0,
      -1,
...
local function load_metadata(buf)
  ...
  for _, m in ipairs(marks) do
    ...
    local sr, sc = m.sr, clamp_col(
      buf,
      m.sr,
      m.sc
    )
    local er, ec = m.er, clamp_col(
      buf,
      m.er,
      m.ec
    )
...
local function get_label(
  ...
  if not label then
    vim.ui.input(
      { prompt = ("NumHi %s-%d label (empty = none): "):format(
        pal,
        slot
      )
      },
      function(input)
        if input
          and input ~= ""
        then State.labels[pal][slot] = input
        end
      end
    )
  end
...
function C.highlight(slot)
  ...
  if
    #State.history > State.opts.history_max
  then
    table.remove(
    State.history,
    1
...
function C.collect_digits()
  ...
  local function prompt()
    ...
    local hl = (#digits > 0) and ensure_hl(
      pal,
      tonumber(digits)
    )
    or
    "Comment"
    echo(string.format(
      "NumHi %s ◈ slot: %s (1-99)  <CR> confirm  <BS> clear  <ESC> cancel",
      pal,
      txt
    ),
      hl
  ...
  while
    ...
    if
      type(ch) == "number"
    then
      ch = fn.nr2char(ch) end
    if ch:match("%d") and #digits < 2 then
      digits = digits .. ch
      prompt()
    elseif
      ch == "\b" or ch == "\127" then -- clear
      digits = ""
      prompt()
    elseif
      ch == string.char(27) then -- escape
      echo("")
      return
    elseif
      ch == "\r"
...
function C.cycle_palette(step)
  ...
  local chunks = {
    {
      "NumHi → palette " .. State.active_palette .. "  ", "ModeMsg"
    }
  }
  for n = 1, 10 do
    local hl = ensure_hl(
...
function C.show_label_under_cursor()
  ...
  for _, pal in ipairs(State.opts.palettes) do
    ...
    if #marks > 0 then
      ...
      require('numhi.ui').tooltip(
        pal,
        slot,
        label,
        note and note.note or nil,
        note and note.tags or nil
      )
      return
    end
  end
